...
def print_model(x_sol,z_sol,solutionname):
    original_stdout = sys.stdout
    with open('{} solution.txt'.format(str(solutionname)).replace(" ","_"), 'w') as f:
        sys.stdout = f
        print('DATASET = ORTEC Caroline VeRoLog 2019')
        print('NAME = {} solution\n'.format(str(solutionname)))
        ...
        for dayNr in range(1,z['t'].max()+1):
            ...
            for key in day_route_trucks:
                i += 1
                truck_string = str(i)
                for value in day_route_trucks[key]:
                    if value == 'depot':
                        truck_string += ' ' + '0'
                    else:
                        truck_string += ' ' + value.replace('R', '')
                print(truck_string)  
                
            print("NUMBER_OF_TECHNICIANS = {}".format(len(day_route_tech)))

            for key in day_route_tech:
                tech_string = str(key.replace('S', ''))
                for value in day_route_tech[key]:
                    if value == 'depot':
                        tech_string += ' ' + '0'
                    else:
